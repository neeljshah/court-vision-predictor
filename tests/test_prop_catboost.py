"""
test_prop_catboost.py — Tests for CatBoost prop trainer and base-learner registry.
"""
...
def test_catboost_importable() -> None:
    """train_props_catboost and _build_prop_training_frame are importable."""
    from src.prediction.player_props import train_props_catboost, _build_prop_training_frame  # noqa: F401


def test_base_learner_registry_catboost() -> None:
    """BASE_LEARNERS contains catboost, xgboost, and lightgbm entries."""
    from src.prediction.prop_model_stack import (
        BASE_LEARNERS,
        predict_base_learner,
        base_learner_available,
    )
    assert "catboost" in BASE_LEARNERS
...
def test_train_catboost_synthetic(monkeypatch: pytest.MonkeyPatch, tmp_path) -> None:
    """train_props_catboost produces correct outputs on synthetic data."""
    ...
    result = player_props.train_props_catboost(force=True)
    ...
    # All 7 .cbm files must exist
    for stat in player_props._PROP_STATS:
        cbm_path = tmp_path / f"props_cb_{stat}.cbm"
        assert cbm_path.exists(), f"Missing model file: {cbm_path}"

    # Metrics JSON must exist
    metrics_path = tmp_path / "props_cb_metrics.json"
    assert metrics_path.exists(), "props_cb_metrics.json not written"

    import json
    with open(metrics_path) as f:
        metrics = json.load(f)
    assert metrics["model"] == "catboost"
    assert "trained_at" in metrics
    assert "stats" in metrics


def test_catboost_no_op_when_models_exist(monkeypatch: pytest.MonkeyPatch, tmp_path) -> None:
    """train_props_catboost returns {} without retraining when models exist and force=False."""
    # Pre-create dummy .cbm files so the early-exit guard triggers
    for stat in player_props._PROP_STATS:
        (tmp_path / f"props_cb_{stat}.cbm").write_bytes(b"dummy")

    monkeypatch.setattr(player_props, "_MODEL_DIR", str(tmp_path))

    result = player_props.train_props_catboost(force=False)
    assert result == {}, "Expected empty dict when models exist and force=False"
