"""scripts/train_residual_heads_endq1.py -- R4-A residual heads (endQ1).

For each of 7 stats, trains a LightGBM head on the RESIDUAL between the
live_engine projection (at endQ1) and the actual full-game total.

Features (14):
  cur_pts, cur_reb, cur_ast, cur_fg3m, cur_stl, cur_blk, cur_tov, cur_pf,
  min_through_q1, score_margin_abs, is_leading, pos_C, pos_F, pos_G

Training set: endQ1 snapshots from player_quarter_stats.parquet.
Gate: WF 4-fold GroupKFold by game_id (chronological). Save .lgb only if
WF mean MAE < zero-prediction baseline on >= 3/4 folds.

Usage:
    python scripts/train_residual_heads_endq1.py
    python scripts/train_residual_heads_endq1.py --max-games 200
"""
...
OUT_DIR = os.path.join(PROJECT_DIR, "data", "models", "residual_heads_endq1")
...
def build_rows(
    ...
    """Build feature matrix X and per-stat residual vectors Y from endQ1 snapshots.
...
def main() -> int:
    ap = argparse.ArgumentParser(description="Train residual heads for R4-A angle (endQ1).")
    ...
    print("  building feature rows (endQ1 snapshots) ...")
    X_rows, Y, game_ids = build_rows(qstats_df, args.max_games, positions)
    print(f"  total rows: {len(X_rows)}")

    report = {"snapshot_point": "endQ1", "trained_stats": [], "skipped_stats": []}
    for stat in STATS:
        y = Y[stat]
        if len(y) < 200:
