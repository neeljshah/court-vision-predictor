"""scripts/train_residual_heads.py -- R2-F residual heads (loop 5).

For each of 7 stats, trains a LightGBM head on the RESIDUAL between the
cycle-110 live_engine projection (at endQ3) and the actual full-game total.

Features (14):
  cur_pts, cur_reb, cur_ast, cur_fg3m, cur_stl, cur_blk, cur_tov, cur_pf,
  min_through_q3, score_margin_abs, is_leading, pos_C, pos_F, pos_G
...
Usage:
    python scripts/train_residual_heads.py
    python scripts/train_residual_heads.py --max-games 200
"""
...
def train_one_stat(
    ...
    for fi in range(4):
        ...
        tr_game_set = set(unique_games[:lo]) | set(unique_games[hi:])
...
def main() -> int:
    ap = argparse.ArgumentParser(description="Train residual heads for R2-F angle.")
