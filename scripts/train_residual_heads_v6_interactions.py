"""scripts/train_residual_heads_v6_interactions.py -- R5-H residual heads v6 (loop 5).

Mirrors train_residual_heads.py but adds 7 engineered interaction features
on top of the 14 R2_F features (21 total):

  15. foul_rate         = cur_pf / max(min_through_q3, 1.0)
  16. pts_per_min       = cur_pts / max(min_through_q3, 1.0)
  17. usage_proxy       = (cur_pts + cur_ast + cur_tov) / max(min_through_q3, 1.0)
  18. margin_sign       = sign(home_score - away_score)  (player-perspective)
  19. min_per_period    = min_through_q3 / 3.0
  20. blowout_flag      = 1 if score_margin_abs > 15 else 0
  21. q3_pace_proxy     = (home_score + away_score) / 36.0

Target: actual_final - BASELINE(endQ3_snap)[stat]  (residual over R2_F baseline)

Gate: save data/models/residual_heads_v6_interactions/{stat}.lgb only if
WF mean MAE strictly < zero-prediction on >= 3/4 folds.

Usage:
    python scripts/train_residual_heads_v6_interactions.py
    python scripts/train_residual_heads_v6_interactions.py --max-games 200
"""
...
OUT_DIR = os.path.join(PROJECT_DIR, "data", "models", "residual_heads_v6_interactions")
...
# 14 base features (R2_F) + 7 interaction features = 21 total
FEATURE_NAMES = [
    # --- 14 base (R2_F) ---
...
def build_rows(
    ...
    """Build 21-feature matrix X and per-stat residual vectors Y.

    Returns:
        X:        List of 21-float feature rows (parallel to game_ids).
...
def main() -> int:
    ap = argparse.ArgumentParser(
        description="Train R5-H residual heads with 21 interaction features."
    )
