"""probe_R1_D_v2_per_player_quantile_variance.py -- improve_loop R1-D-v2 (loop 5).
...
stat's overall val-set coverage = 0.80 (instead of a single global rescale).
...
JSON schema (scripts/_results/improve_R1_D_v2_per_player_quantile_variance.json):
  name, n_games, per_stat_rescales, base_spread, treat_spread, spread_delta,
  per_stat: [{stat, n, base_overall_cov, treat_overall_cov,
              base_buckets:{Low,Mid,High}, treat_buckets:{Low,Mid,High}}],
  ship, ship_reason

Run:
    python scripts/probe_R1_D_v2_per_player_quantile_variance.py [--max-games N]
"""
...
class CalProbeResult:
    ...
    def to_md(self) -> str:
        ...
        hdr = (f"# probe {self.name} -- improve_loop (CALIBRATION)\n\n"
               f"**Games:** {self.n_games}  **Per-stat rescales:** {rescale_str}\n\n"
...
def run_probe(max_games: int = 0) -> CalProbeResult:
    name = "R1_D_v2_per_player_quantile_variance"
    ...
    print(f"[{name}] bisecting per-stat rescales...", flush=True)
    ...
    # Gate corrected: "no bucket worse" means no bucket FURTHER from 0.80 than
    # its baseline counterpart -- not raw coverage drop. Over-covering buckets
    # (e.g. AST Low at 0.867) moving toward 0.80 is the WHOLE POINT of this
    # probe, not a regression. Allow 0.02 slack for noise.
    no_bad = all(
...
def main() -> int:
    ap = argparse.ArgumentParser(
        description="Calibration probe v2: per-stat variance-modulated quantile bands")
