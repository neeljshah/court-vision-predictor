"""scripts/probe_R4_A_residual_heads_endq1.py -- R4-A residual heads probe (endQ1).

For each (pid, stat) at endQ1, if data/models/residual_heads_endq1/{stat}.lgb exists,
...
Usage:
    from scripts.probe_R4_A_residual_heads_endq1 import treatment
    from scripts.improve_loop.scaffold import run_point_probe, BASELINE
    run_point_probe("endQ1", "R4_A_residual_heads_endq1", treatment, baseline=BASELINE)

CLI:
    python scripts/probe_R4_A_residual_heads_endq1.py
    python scripts/probe_R4_A_residual_heads_endq1.py --max-games 100
"""
...
HEAD_DIR = os.path.join(PROJECT_DIR, "data", "models", "residual_heads_endq1")

FEATURE_NAMES = [
    "cur_pts", "cur_reb", "cur_ast", "cur_fg3m",
    "cur_stl", "cur_blk", "cur_tov", "cur_pf",
    "min_through_q1", "score_margin_abs", "is_leading",
    "pos_C", "pos_F", "pos_G",
]
...
def _load_heads() -> Dict[str, object]:
    """Load all available .lgb heads from residual_heads_endq1/ into memory (once)."""
...
def treatment(snap: dict) -> Dict[Tuple[int, str], float]:
    """Apply endQ1 residual head corrections on top of BASELINE projections.
    ...
    for player in snap.get("players", []):
        ...
        feat = np.array([[
            ...
            float(player.get("min_q1", 0)),   # min_through_q1 == min_q1 at endQ1
            margin,
            float(raw_margin > 0),
            pos_c,
            pos_f,
...
def main() -> int:
    ap = argparse.ArgumentParser(description="R4-A residual heads probe (endQ1).")
    ...
    if n_heads == 0:
        print("  No residual heads found. Run train_residual_heads_endq1.py first.")
        return 1

    print(f"  {n_heads} head(s) found. Running probe ...")
    run_point_probe(
        "endQ1",
        "R4_A_residual_heads_endq1",
        treatment,
        baseline=BASELINE,
