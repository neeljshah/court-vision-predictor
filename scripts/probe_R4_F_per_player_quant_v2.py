"""probe_R4_F_per_player_quant_v2.py -- improve_loop R4-F (loop 5).

CALIBRATION probe. Per-player variance-modulated quantile bands at endQ1
(currently no calibration, placeholder bands) AND refreshed endQ2 calibration
(R3-A residual heads wired → residuals tighter since last calibration).

Treatment: half = base_sigma*scale*Z80 * per_stat_rescale[stat, point]
           * sqrt(clip(std_l20/pop_mean_std, 0.6, 1.8))
Per-stat rescale: each of the 7 stats is independently bisected so that
stat's overall val-set coverage = 0.80 at that snapshot point.

Evaluation: bucket (pid, game) rows by std_l20 tercile, compute 80% empirical
coverage per (stat, bucket, point). Primary metric: avg across 7 stats of
(max_bucket_cov - min_bucket_cov) = "spread".

Ship gate (per point):
  spread reduction >= 0.03 at EACH of endQ1 and endQ2
  AND all stat overall coverage in [0.78, 0.82] per (stat, point)
  AND no bucket worse than baseline by > 0.03 distance from 0.80

Output:
  scripts/_results/improve_R4_F_per_player_quant_v2.{md,json}
  data/models/per_player_quantile_calibration_v2.json  (NEW -- endQ1+endQ2)

Run:
    python scripts/probe_R4_F_per_player_quant_v2.py [--max-games N]
"""
...
_TARGET_COV = 0.80
_L20 = 20
_PROBE_NAME = "R4_F_per_player_quant_v2"

# Minimum spread reduction required per point to ship
...
# ── corpus collection (per snapshot point) ────────────────────────────────────

CorpusData = Tuple[
    Dict[str, np.ndarray],        # q50s
    Dict[str, np.ndarray],        # acts
    Dict[str, List[Optional[float]]],  # stds
    int,                          # n_games
]


def collect(point: str, max_games: int, idx: dict) -> CorpusData:
    """Collect projected + actual stat rows for a given snapshot point."""
    ...
    for gid in gids:
        ...
        n_ok += 1
        if n_ok % 100 == 0:
            print(f"  [{point} corpus] {n_ok}/{len(gids)}", flush=True)
    print(f"  [{point} corpus] {n_ok} games, pts n={len(q50s['pts'])}", flush=True)
    return (
...
def evaluate_point(point: str, q50s, acts, stds, n_games, pmstds, full_cal) -> PointResult:
    cal = (full_cal or {}).get(point, {})
    print(f"\n[{_PROBE_NAME}] bisecting per-stat rescales at {point}...", flush=True)
    ...
    no_bad = all(
        abs(tb_v - 0.80) <= abs(bb_v - 0.80) + 0.03
        for r in per_stat
    ...
    if not no_bad:
        causes.append("bucket degraded >0.03 vs baseline")
    ship_reason = (
        f"spread {avg_t:.4f}<={avg_b:.4f}-{_MIN_SPREAD_REDUCTION}; cov in range; no bad bucket"
        if ship else "; ".join(causes) or "gate not met"
    )

    print(f"  [{point}] SHIP={ship}  base_spread={avg_b:.4f}  "
          f"treat_spread={avg_t:.4f}  delta={delta:+.4f}", flush=True)

    return PointResult(
        point=point, n_games=n_games,
...
def _point_md_section(pr: PointResult) -> str:
    rescale_str = "  ".join(f"{s}={v:.4f}" for s, v in pr.per_stat_rescales.items())
    hdr = (f"## Point: {pr.point} (games={pr.n_games})\n\n"
           f"**Per-stat rescales:** {rescale_str}\n\n"
           "### Spread\n\n| metric | baseline | treatment | delta |\n"
           "|--------|----------|-----------|-------|\n"
           f"| spread (avg max-min cov) | {pr.base_spread:.4f} "
           f"| {pr.treat_spread:.4f} | {pr.spread_delta:+.4f} |\n\n"
           "### Per-stat bucket coverage\n\n"
           "| stat | base_all | treat_all | bLow | bMid | bHigh"
           " | tLow | tMid | tHigh |\n"
           "|------|----------|-----------|------|------|------|------|------|------|\n")
    rows = []
    for r in pr.per_stat:
    ...
    verdict = f"\n**Verdict:** {'SHIP' if pr.ship else 'REJECT'} -- {pr.ship_reason}\n"
    return hdr + "\n".join(rows) + verdict


def build_md(point_results: List[PointResult], overall_ship: bool, pmstds: Dict[str, float]) -> str:
    pop_str = "  ".join(f"{s}={pmstds[s]:.3f}" for s in STATS)
    top = (f"# probe {_PROBE_NAME} -- improve_loop R4-F (CALIBRATION)\n\n"
           f"**Points:** {', '.join(SNAPSHOT_POINTS)}\n"
           f"**Pop mean stds:** {pop_str}\n\n")
    sections = "\n\n".join(_point_md_section(pr) for pr in point_results)
    final = f"\n\n## Overall Verdict: {'SHIP' if overall_ship else 'REJECT'}\n"
    return top + sections + final


# ── v2 calibration JSON ────────────────────────────────────────────────────────

def write_v2_calibration(point_results: List[PointResult], pmstds: Dict[str, float],
                          existing_cal: dict) -> None:
    """Merge endQ1+endQ2 new rescales into a v2 calibration file.

    Schema mirrors per_player_quantile_calibration.json but adds endQ1 block
    and overwrites endQ2 block. endQ3 is preserved unchanged from existing cal.
    pop_mean_std is shared across points (computed from the same corpus).
    """
    v2: dict = {}

    # Preserve any existing points not being recalibrated (e.g. endQ3)
    for pt, pt_data in (existing_cal or {}).items():
        if pt not in SNAPSHOT_POINTS:
            v2[pt] = pt_data

    for pr in point_results:
        pt_block: dict = {"per_stat_rescale": {}}
        for s in STATS:
            pt_block["per_stat_rescale"][s] = pr.per_stat_rescales[s]
        pt_block["pop_mean_std"] = {s: round(pmstds[s], 6) for s in STATS}
        pt_block["target_coverage"] = _TARGET_COV
        pt_block["ship"] = pr.ship
        pt_block["base_spread"] = pr.base_spread
        pt_block["treat_spread"] = pr.treat_spread
        v2[pr.point] = pt_block

    os.makedirs(_MODELS_DIR, exist_ok=True)
    with open(_V2_CAL_PATH, "w", encoding="utf-8") as fh:
        json.dump(v2, fh, indent=2)
    print(f"  wrote {_V2_CAL_PATH}", flush=True)


# ── main ──────────────────────────────────────────────────────────────────────

def run_probe(max_games: int = 0) -> List[PointResult]:
    ...
    full_cal = load_calibration() or {}
    point_results: List[PointResult] = []

    for point in SNAPSHOT_POINTS:
        print(f"\n[{_PROBE_NAME}] collecting corpus at {point}...", flush=True)
        q50s, acts, stds, n_games = collect(point, max_games, idx)
        pr = evaluate_point(point, q50s, acts, stds, n_games, pmstds, full_cal)
        point_results.append(pr)

    overall_ship = all(pr.ship for pr in point_results)
    print(f"\n[{_PROBE_NAME}] OVERALL SHIP={overall_ship} "
          f"({'all points passed' if overall_ship else 'one or more points rejected'})",
          flush=True)
    ...
    with open(md_path, "w", encoding="utf-8") as fh:
        fh.write(build_md(point_results, overall_ship, pmstds))
    ...
    summary = {
        ...
        "overall_ship": overall_ship,
        "point_results": [_clean(asdict(pr)) for pr in point_results],
    }
    ...
    # Always write v2 calibration so rescales are available even if gate fails
    write_v2_calibration(point_results, pmstds, full_cal)
...
def main() -> int:
    ap = argparse.ArgumentParser(
        description="Calibration probe R4-F: per-player variance-modulated quantile bands "
                    "at endQ1 (new) + endQ2 (refreshed)")
    ap.add_argument("--max-games", type=int, default=0,
                    help="Cap number of games per point (0 = all)")
    args = ap.parse_args()
