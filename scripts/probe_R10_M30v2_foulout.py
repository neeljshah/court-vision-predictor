...
# Dampener: -5% volume, -3% other
DAMPENER_VOLUME = 0.95
DAMPENER_OTHER  = 0.97

THRESHOLD = 0.20   # P(pf_final>=5) > threshold → dampen
...
for fold_i, (train_games, test_games) in enumerate(fold_splits):
    ...
    for s in ALL_STATS:
        ...
        # Dampened: apply multiplier where p_foulout > threshold
        multiplier = DAMPENER_VOLUME if s in VOLUME_STATS else DAMPENER_OTHER
...
results = {
    ...
    "stage2": {
        "dampener_volume": DAMPENER_VOLUME,
        "dampener_other": DAMPENER_OTHER,
        "folds": [
            {
                "fold": r["fold"],
...
out_path = ROOT / "data" / "cache" / "probe_R10_M30v2_foulout_results.json"
out_path.parent.mkdir(parents=True, exist_ok=True)
with open(out_path, "w") as f:
    json.dump(results, f, indent=2)

print(f"\n[8] Results saved to {out_path}")
print(f"\n=== PROBE R10_M30v2 COMPLETE ===")
