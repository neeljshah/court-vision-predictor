"""sweep_reg_lambda.py — per-stat L2 (reg_lambda) sweep for prop_pergame.

Set per-stat originally (cycle 12/20) but never swept since the other
regs landed. The new interactions may push optimal lambdas.
"""
...
# Current per-stat params (post cycle 29).
_CURRENT = {
...
def main():
    ...
    results = {}
    for stat in STATS:
        print(f"\n[{stat}] sweeping reg_lambda {_GRID}")
        results[stat] = {}
        for rl in _GRID:
            params = dict(_CURRENT[stat], reg_lambda=rl)
            ...
            results[stat][rl] = (mae, r2)
            print(f"  {stat:5} reg_lambda={rl:.2f} -> MAE {mae:.4f}  R² {r2:.4f}")
    ...
    for stat in STATS:
        ...
        print(f"  {stat:5} winner rl={best:.2f}  MAE {mae:.4f} ({(mae-cm)/cm*100:+.3f}%)  R²Δ{r2-cr:+.4f}")

    print(f"\nIf every winner ships: net MAE {net_mae:+.4f}, net R² {net_r2:+.4f}")

    out = os.path.join(_MODEL_DIR, "reg_lambda_sweep_results.json")
