"""retrain_qstat_q50_oos_seeded.py - iter-20 seed-stability adaptation.

Adapted from iter-7's retrain_qstat_q50_oos.py to accept --seed and write to
data/models/oos_pre_playoffs_seed<N>/. Used to check whether iter-18's
|edge|=0.35 PnL optimum is a seed-noise artifact.

REB -> LGB-q50 (skipped here; iter-18 only uses BLK/FG3M/STL/TOV).
BLK/FG3M/STL/TOV -> XGB-q50.

Usage: python scripts/retrain_qstat_q50_oos_seeded.py --stat blk --seed 7
"""
...
def _train_xgb(X_tr, X_val, yt_tr, yt_val, sw, params, seed):
...
def _train_lgb(X_tr, X_val, yt_tr, yt_val, sw, params, seed):
    ...
    m = lgb.LGBMRegressor(
        ...
        random_state=seed, seed=seed, objective="quantile", alpha=0.5,
...
def main():
    ...
    print(f"  [stat={stat} seed={seed}] method={method}  out_dir={out_dir}")
    ...
    if method == "lgb":
        m, fit_secs, best_iter = _train_lgb(X_tr, X_val, yt_tr, yt_val, sw, params, seed)
    else:
        m, fit_secs, best_iter = _train_xgb(X_tr, X_val, yt_tr, yt_val, sw, params, seed)
