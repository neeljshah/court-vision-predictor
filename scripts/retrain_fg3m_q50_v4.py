"""retrain_fg3m_q50_v4.py — cycle 101c (loop 5).

FG3M LGB-q50 retrain with the FULL candidate union now that the q1_fg3m_l5
coverage gap (cycles 99b/100c blocker) has been closed.

Cycle 99b (v2) attempted q1_fg3m_l5 + home_spread + position_C/F/G; q1 had
0% holdout coverage so the wide model added nothing useful and the run
REJECT'd. Cycle 100c (v3) pivoted to the 4 cycle-99e opp-context L5
features (opp_team_def_rtg_l5, opp_team_pace_l5, opp_team_ts_pct_l5,
opp_def_fg3m_l5) — passed single-split (-0.0024 MAE) but failed walk-forward
2/4 folds (only fold 1 was a meaningful win, folds 2-3 regressed slightly).

v4 retries the FULL union once q1_fg3m_l5 coverage reached ~85% per the
runbook (per-quarter daemon backfill from cycle df36c17f). Candidate set:

  opp_team_def_rtg_l5    — opp defensive rating L5
  opp_team_pace_l5       — opp pace L5
  opp_team_ts_pct_l5     — opp true-shooting allowed L5
  opp_def_fg3m_l5        — opp raw FG3M allowed L5
  q1_fg3m_l5             — rolling-5 prior Q1 FG3M (NOW UNLOCKED)
  position_C / F / G     — one-hot triplet from cycle 90e
  home_spread            — pre-game spread (player perspective, cycle 95a)

= up to 9 additive features layered on the 85-col baseline (so ~94 wide
cols if every feature clears the 30% holdout coverage gate; the runbook
quoted ~93 features expected — small drift either way is fine, the gate
prunes anything thin).
...
When passing: save the wider model to data/models/fg3m_q50_v4.pkl. Production
dispatch wire-in updates _load_q50_model to prefer fg3m_q50_v4.pkl over
v3.pkl over the default quantile_pergame_fg3m_q50.json, and feature_columns()
gets a global extension (NOT stat-specific — coordinated with sibling cycles).

Coverage gate: 30% holdout (matches v2/v3). Below 30% → drop that feature.
"""
...
# v4 union: cycle-99e opp-context L5 quartet PLUS the v2 trio (q1, spread,
# position one-hots) now that q1 coverage is no longer the 0% blocker.
CANDIDATE_FEATURES = (
    "opp_team_def_rtg_l5",
    "opp_team_pace_l5",
    "opp_team_ts_pct_l5",
...
# COVERAGE_KEY maps each candidate to the field used to count holdout
# coverage. opp/q1/home_spread map 1:1 to their own key; the position
# one-hots share a single underlying "position" string source.
COVERAGE_KEY = {
    "opp_team_def_rtg_l5": "opp_team_def_rtg_l5_pct",
    "opp_team_pace_l5":    "opp_team_pace_l5_pct",
    "opp_team_ts_pct_l5":  "opp_team_ts_pct_l5_pct",
    "opp_def_fg3m_l5":     "opp_def_fg3m_l5_pct",
    "q1_fg3m_l5":          "q1_fg3m_l5_pct",
    "home_spread":         "home_spread_pct",
...
def _position_onehot(pos: object) -> Tuple[float, float, float]:
    ...
    Mirrors v2: multi-position strings like 'Guard-Forward' set BOTH bits
    so a Guard-Forward player carries position_F=1 and position_G=1.
    Unknown / None positions produce (0, 0, 0).
    """
    if pos is None:
        return 0.0, 0.0, 0.0
    s = str(pos)
...
def _coerce_float(v) -> float:
    """None / non-finite / non-numeric → 0.0; otherwise float(v)."""
    try:
...
def _augment_row(row: dict) -> dict:
    """Return a shallow-copied row with all 9 candidate features filled.

    Every key in CANDIDATE_FEATURES lands as a finite float so LGB never
    sees None / NaN. Missing source values collapse to 0.0 — same null
    semantics as v2/v3.
    """
    new = dict(row)
    # Numeric features.
    for k in ("opp_team_def_rtg_l5", "opp_team_pace_l5", "opp_team_ts_pct_l5",
              "opp_def_fg3m_l5", "q1_fg3m_l5", "home_spread"):
        new[k] = _coerce_float(row.get(k))
    # Position one-hots.
...
def _coverage_report(rows: List[dict]) -> dict:
    ...
    Position counts as covered when row['position'] is a non-empty string.
    """
    n = len(rows)
    if n == 0:
        return {"n_rows": 0}
    ...
    pos_present = sum(1 for r in rows if r.get("position"))
    out["position_pct"] = round(100.0 * pos_present / n, 2)
    return out


def _fg3m_params() -> dict:
    """Match prop_quantiles._per_stat_xgb_params('fg3m') overrides — same
    recipe as v2/v3 so the baseline arm is comparable to cycle 27's
    persisted XGB-q50 model."""
...
def main() -> int:
    t0 = time.time()
    print("[cycle 101c] FG3M q50 retrain v4 with q1_fg3m_l5 UNLOCKED + full union",
          flush=True)
    print("=" * 72, flush=True)
    ...
    print(f"  wide               : MAE={ss['mae_wide']:.4f}", flush=True)
    ...
    if single_split_pass and wf_pass:
        import joblib
        path = os.path.join(_MODEL_DIR, "fg3m_q50_v4.pkl")
