"""retrain_iter48_momentum_delta.py — Iter-48 OOS retrain: momentum-delta features.

Probe: add mom_delta_X = l3_X - l5_X for PTS, AST, REB only.
Positive value = hot streak (l3 average > l5 average).
Negative value = cooling trend.

Rationale: Iter-47 proved l3 raw is collinear with existing l5/l10/ewma —
the model already has enough information to learn the same split internally
but the explicit delta provides a direct shortcut that avoids the
collinearity overhead in the tree-split search.

New feature counts (mom_delta appended after existing iter-47 reverted block):
  PTS : 129 → 130  (+mom_delta_pts)
  AST : 129 → 130  (+mom_delta_ast)
  REB : 132 → 133  (+mom_delta_reb)  [REB was 132 with oreb-context keys]
...
Usage:
    python scripts/retrain_iter48_momentum_delta.py
...
def _print_mom_delta_stats(rows: list, stat: str) -> None:
    """Sanity-check: print mean/std of mom_delta feature in training data."""
    key = f"mom_delta_{stat}"
    vals = [float(r.get(key, 0.0) or 0.0) for r in rows]
    if not vals:
        print(f"  [sanity] {key}: NO DATA", flush=True)
        return
    arr = np.array(vals)
    print(f"  [sanity] {key}: mean={arr.mean():.4f}  std={arr.std():.4f}  "
          f"min={arr.min():.4f}  max={arr.max():.4f}  n={len(arr)}", flush=True)
...
def retrain_pts(rows: list) -> dict:
    ...
    new_cols = [c for c in fcols if c.startswith("mom_delta_")]
    print(f"\n  [iter-48] {stat}: {len(fcols)} cols  new={new_cols}", flush=True)
    _print_mom_delta_stats(rows, stat)
    ...
    for rank, (name, imp) in enumerate(top10_xgb, 1):
        marker = " <-- NEW" if name.startswith("mom_delta_") else ""
...
def main() -> None:
    print(f"\n=== iter-48 retrain: momentum-delta (l3-l5) for PTS/AST/REB "
    ...
    # Sanity check: verify mom_delta columns are present on rows
    sample_row = rows[0] if rows else {}
    for stat in ("pts", "ast", "reb"):
        key = f"mom_delta_{stat}"
        if key not in sample_row:
            print(f"  [ERROR] {key} not found on row — check prop_pergame.py edits!", flush=True)
            sys.exit(1)
        else:
            print(f"  [OK] {key} present on rows", flush=True)
    ...
    print(f"\n=== iter-48 retrain complete in {elapsed:.1f}s ===", flush=True)
    for r in results:
        stat = r["stat"]
        if stat == "reb":
