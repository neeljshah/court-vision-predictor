...
OUT_JSON = os.path.join(CACHE_DIR, "probe_R11_volatility_std_results.json")
...
def build_features_and_targets(
    ...
    form_feats: List[str] = []
    vol_feats: List[str] = []  # R11 NEW DATA: L10 std (volatility) per stat
    for stat in STATS:
        ...
        # R11 volatility addition: L10 stat std as new feature (NEW DATA TYPE).
        # Computed separately so we can ablate base vs base+vol cleanly.
        pergame[f"l10std_{stat}"] = shifted.groupby(pergame["player_id"]).transform(
            lambda s: s.rolling(10, min_periods=2).std().fillna(0.0)
        )
        form_feats += [f"l5_{stat}", f"l10_{stat}", f"ewma_{stat}"]
        vol_feats += [f"l10std_{stat}"]
    ...
    # R11 volatility probe: NO playtype join — we test base vs base + vol_feats.
    print(f"  Volatility feature set ({len(vol_feats)} cols): {vol_feats}", flush=True)

    return pergame.reset_index(drop=True), base_feat_cols, vol_feats
...
def main():
    t_start = time.time()
    print("=" * 60, flush=True)
    print("Probe R11_volatility: L10 stat std as new feature (NEW DATA)", flush=True)
    print("=" * 60, flush=True)
    ...
    print("\n[2] Building features (rolling form + L10 std volatility) ...", flush=True)
    # Pass dummy playtype args (unused in this build_features variant)
    merged, base_cols, vol_cols = build_features_and_targets(pergame, None, [])
    print(f"  Final dataset rows: {len(merged)}", flush=True)
    print(f"  Base features: {len(base_cols)}", flush=True)
    print(f"  Volatility features: {len(vol_cols)}: {vol_cols}", flush=True)
    # Rebind for downstream code that expects pt_cols name
    pt_cols = vol_cols
    ...
    # Write results JSON
    result_doc = {
        "probe": "R11_volatility_std",
        "description": "L10 stat std (volatility) as new feature on per-game prop prediction",
        "leakage_discipline": "shift(1).rolling(10).std() — strict prior-game volatility",
        "n_rows": int(len(merged)),
        "base_features": base_cols,
        "vol_features": pt_cols,
        "ship_gate": {
