...
OUT_JSON = os.path.join(DATA_CACHE, "probe_R11_M2v3_home_pts_results.json")
...
def main():
    t0 = time.time()
    print("=" * 60, flush=True)
    print("probe_R11_M2v3_home_pts - game-level HOME team total points", flush=True)
    print("=" * 60, flush=True)
    ...
    y = merged["home_score"].astype(float).values

    # Naive baseline: rolling L5 mean of home_score (strictly prior games)
    print("\n[4] Naive baseline: L5 rolling mean of home_score ...", flush=True)
    naive_pred = merged["home_score"].shift(1).rolling(5, min_periods=1).mean().fillna(
        merged["home_score"].mean()
    ).values
