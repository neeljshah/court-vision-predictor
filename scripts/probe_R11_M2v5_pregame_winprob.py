...
OUT_JSON = os.path.join(DATA_CACHE, "probe_R11_M2v5_pregame_winprob_results.json")
...
def main():
    t0 = time.time()
    print("=" * 60, flush=True)
    print("probe_R11_M2v5_pregame_winprob - binary classifier on home_win", flush=True)
    print("=" * 60, flush=True)

    print("\n[1] Loading season_games (pregame features + home_win label) ...", flush=True)
    merged = load_season_games()
    ...
    # home_win must exist as label
    merged = merged[merged["home_win"].notna()]
    ...
    print(f"  home_win rate: {merged['home_win'].mean():.3f}", flush=True)
    ...
    y = merged["home_win"].astype(int).values

    # Naive baseline: sim_win_prob (already in season_games)
    print("\n[4] Naive baseline: sim_win_prob ...", flush=True)
    if "sim_win_prob" in merged.columns:
        naive_pred = merged["sim_win_prob"].fillna(0.55).clip(0.01, 0.99).values
    else:
        naive_pred = np.full(len(merged), 0.55)
    ...
    # Ship gate -- winprob: Brier <= 0.183 AND acc >= 0.72
    print("\n[6] Ship gate ...", flush=True)
    n_valid_folds = len(fold_results)
    gate_brier = pooled_lgb_brier <= 0.183
    gate_acc = pooled_lgb_acc >= 0.72
    ship = gate_brier and gate_acc and n_valid_folds >= 3

    print(f"  Pooled Brier: {pooled_lgb_brier:.4f} (gate <=0.183) -- "
          f"{'PASS' if gate_brier else 'FAIL'}", flush=True)
    print(f"  Pooled Acc:   {pooled_lgb_acc:.4f} (gate >=0.720) -- "
          f"{'PASS' if gate_acc else 'FAIL'}", flush=True)
    print(f"  Pooled AUC:   {pooled_lgb_auc:.4f}", flush=True)
    print(f"  Naive Brier:  {pooled_naive_brier:.4f} (delta {brier_delta_pct:+.2f}%)", flush=True)
    print(f"\n  VERDICT: {'SHIP' if ship else 'REJECT'}", flush=True)

    out = {
        "probe": "R11_M2v5_pregame_winprob",
        "status": "SHIP" if ship else "REJECT",
        "ship_reason": (
            f"Brier {pooled_lgb_brier:.4f} {'pass' if gate_brier else 'fail'} (gate<=0.183); "
            f"Acc {pooled_lgb_acc:.4f} {'pass' if gate_acc else 'fail'} (gate>=0.720); "
            f"vs sim_win_prob naive Brier {pooled_naive_brier:.4f}"
        ),
        "n_games": len(merged),
