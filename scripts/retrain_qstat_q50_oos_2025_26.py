"""retrain_qstat_q50_oos_2025_26.py - iter-12 parametrized-cutoff retrain.

Refactor of retrain_qstat_q50_oos.py with --cutoff flag. Filters
prop_pergame to rows with date < cutoff, retrains the q50 head for one stat
with same HPs+sample weights as production train_quantile_models. Writes to
data/models/oos_pre_2025_26/ by default (production untouched).

REB -> LGB-q50 (cycle 29). FG3M/STL/BLK/TOV -> XGB-q50.
Usage: python scripts/retrain_qstat_q50_oos_2025_26.py --stat blk --cutoff 2025-10-01
"""
...
DEFAULT_MODEL_DIR = os.path.join(PROJECT_DIR, "data", "models", "oos_pre_2025_26")
LGB_STATS = {"reb"}
...
def main():
    ...
    ap.add_argument("--cutoff", default=DEFAULT_CUTOFF,
                    help="ISO date; rows strictly before are train+val (default 2025-10-01)")
    ap.add_argument("--out_dir", default=DEFAULT_MODEL_DIR)
    args = ap.parse_args()
    stat = args.stat
    ...
    print(f"  [stat={stat}] method={method} cutoff={cutoff_date}")
