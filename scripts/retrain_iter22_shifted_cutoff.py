"""retrain_iter22_shifted_cutoff.py — Iter-22 architectural change: shift training cutoff.

GOAL: train candidate models with cutoff 2025-04-21 (includes 2024-25 RS + playoffs)
and validate ONLY on 2025-26 RS + 2025-26 playoffs (genuinely OOS).

Compare to production's 2025-26 ROI. Ship if 4+/7 stats improve by >=+1pp.

Usage:
    python scripts/retrain_iter22_shifted_cutoff.py [--ship]

Outputs:
    data/models/oos_pre_playoffs/_candidate_shifted_cutoff/<artifacts>
    data/cache/iter22_comparison.json
"""
...
# ─── local player resolver (fallback when nba_api unavailable) ────────────────
...
def _build_local_player_index() -> Dict[str, int]:
    """Build name->pid map from data/cache/playerinfo/*.json (850 players)."""
...
def _local_resolve_player_id(name: str) -> Optional[int]:
    """Resolve player name -> id using local playerinfo cache.

    Resolution order:
    1. Exact (accent-normalised) match.
    2. Substring containment in either direction (handles Jr./Sr./II suffix variants).
    3. Last-token (surname) match as last resort.
    """
...
NEW_CUTOFF_DATE = "2025-04-21"
PROD_CUTOFF_DATE = "2024-04-21"

OOS_PROD_DIR = os.path.join(PROJECT_DIR, "data", "models", "oos_pre_playoffs")
CANDIDATE_DIR = os.path.join(OOS_PROD_DIR, "_candidate_shifted_cutoff")
...
SHIP_MIN_STATS = 4         # need 4+/7 stats
...
# Production's known 2025-26 ROI (from holdout_baseline.json __global__)
# These are the combined 4-slice numbers; we need just 2025-26 slice.
# Will be computed fresh from the CSV files for fair comparison.
PROD_KNOWN_2025_26 = {
    # from MEMORY.md: RS 722 bets +16.61% pool, playoffs 889 bets +11.71% pool
    # => pool avg ~+13.9% on 1611 bets. We'll recompute per-stat from disk.
}
...
def _get_dataset_at_cutoff(cutoff_date: str):
    """Return (pre_rows, fcols, n_all) filtered to rows before cutoff_date."""
...
def _predict_ast_blend(artifacts: dict, feat_row: dict) -> Optional[float]:
    """AST blend: log1p multitask MLP. Similar predict path to pts but log1p inverse."""
    cols = feature_columns()
    X = np.array([[float(feat_row.get(c, 0.0) or 0.0) for c in cols]], dtype=float)
    weights = artifacts.get("weights")
    if not weights:
...
def _resolve_player_id(name: str) -> Optional[int]:
    """Resolve player name -> id.  Uses nba_api when available, falls back to
    the local playerinfo cache (850 players) otherwise.
    """
...
def run_backtest_2025_26(model_dir: str, label: str) -> dict:
    """Run backtest on 2025-26 RS + playoffs slices, using models in model_dir."""
    print(f"\n{'='*65}")
    print(f"  BACKTEST [{label}] on 2025-26 (RS + Playoffs)")
    ...
    for csv_path in SLICES_2025_26:
        ...
        with open(csv_path, encoding="utf-8") as fh:
            for r in csv.DictReader(fh):
                ...
                if stat in models:
                    r["_slice"] = os.path.basename(csv_path)
                    all_rows.append(r)
    print(f"  Total 2025-26 CSV rows: {len(all_rows)}")
    if not all_rows:
...
def compare_and_decide(prod_results: dict, cand_results: dict) -> dict:
    """Compare per-stat ROI for prod vs candidate on 2025-26 only."""
    ...
    for s in stats_order:
        ...
        delta = c_roi - p_roi
        improved = (c_n >= 30 and delta >= SHIP_THRESHOLD_PP)
        rows.append({
            "stat": s,
            "prod_roi": p_roi,
            "cand_roi": c_roi,
    ...
    print(f"\n{'='*70}")
    print(f"  ITER-22 COMPARISON — Production (cutoff {PROD_CUTOFF_DATE}) "
          f"vs Candidate (cutoff {NEW_CUTOFF_DATE})")
    print(f"  Validation slice: 2025-26 RS + 2025-26 Playoffs")
    print(f"{'='*70}")
    ...
    for row in rows:
        ...
        print(f"  {row['stat'].upper():<8}{row['prod_roi']:>+11.2f}%{row['cand_roi']:>+11.2f}%"
              f"{row['delta_pp']:>+9.2f}pp{row['prod_n_bets']:>8}{row['cand_n_bets']:>8}  {tag}")
    print(f"  {'-'*68}")
    print(f"\n  Improvements: {len(improvements)}/7 stats (+{SHIP_THRESHOLD_PP}pp threshold, "
          f"need {SHIP_MIN_STATS}+): {improvements}")
    print(f"  Regressions:  {regressions}")
    print(f"\n  *** DECISION: {decision} ***")
    if ship:
        print(f"  -> Promote candidate to production (cutoff {PROD_CUTOFF_DATE} -> {NEW_CUTOFF_DATE})")
    else:
        print(f"  -> Keep production models (not enough improvement on 2025-26)")
...
def promote_candidate(candidate_dir: str, prod_dir: str) -> None:
    """Promote candidate artifacts to production directory (swap)."""
    ts = datetime.now().strftime("%Y%m%d_%H%M%S")
    old_backup = os.path.join(os.path.dirname(prod_dir),
                              f"_backup_iter22_promoted_{ts}")
    print(f"\n  Promoting candidate to production...")
    print(f"  Backup old prod -> {old_backup}")

    # Back up only the model files (not subdirs)
    ...
    # Copy candidate artifacts -> prod dir
...
def update_holdout_baseline(new_baseline: dict) -> None:
    """Update data/cache/holdout_baseline.json with new 2025-26 baseline."""
    ...
    # Update 2025-26 slice info in __global__
    if "__global__" not in existing:
        existing["__global__"] = {}
    for s, d in new_baseline.items():
        existing["__global__"][s] = d

    existing["__updated_at__"] = datetime.utcnow().isoformat() + "+00:00"
    if "__source__" not in existing:
        existing["__source__"] = {}
    existing["__source__"]["iter"] = "iter22"
    existing["__source__"]["note"] = "cutoff shifted to 2025-04-21 (includes 2024-25 season)"
...
def main():
    ...
    ap.add_argument("--skip-prod-backtest", action="store_true",
                    help="Skip production backtest (use existing results from cache)")
    args = ap.parse_args()

    t_global = time.time()

    # ── Step 1: Ensure backup exists ──────────────────────────────────────────
    print(f"\n=== ITER-22: Shifted Cutoff Retrain ===")
    print(f"  New cutoff: {NEW_CUTOFF_DATE}  (adds 2024-25 season to training)")
    print(f"  Validate on: 2025-26 RS + 2025-26 Playoffs")
    print(f"  Ship threshold: {SHIP_MIN_STATS}+ stats improve by ≥{SHIP_THRESHOLD_PP}pp ROI")
    ...
    if args.skip_train:
        ...
        cand_meta = {"stats": {}, "cutoff": NEW_CUTOFF_DATE, "iter": "iter22"}
    ...
    # ── Step 3: Run production backtest on 2025-26 ────────────────────────────
    cache_path = os.path.join(PROJECT_DIR, "data", "cache", "iter22_prod_2025_26.json")
    if args.skip_prod_backtest and os.path.exists(cache_path):
        print(f"\n  [SKIP PROD BACKTEST] Loading from {cache_path}")
        prod_results = json.load(open(cache_path, encoding="utf-8"))
    else:
        print(f"\n=== PRODUCTION BACKTEST on 2025-26 ===")
        prod_results = run_backtest_2025_26(OOS_PROD_DIR, "PRODUCTION (cutoff 2024-04-21)")
        with open(cache_path, "w", encoding="utf-8") as fh:
            json.dump(prod_results, fh, indent=2)
        print(f"  Production results cached -> {cache_path}")

    # ── Step 4: Run candidate backtest on 2025-26 ─────────────────────────────
    print(f"\n=== CANDIDATE BACKTEST on 2025-26 ===")
    cand_results = run_backtest_2025_26(CANDIDATE_DIR, "CANDIDATE (cutoff 2025-04-21)")

    # ── Step 5: Compare and decide ────────────────────────────────────────────
    comparison = compare_and_decide(prod_results, cand_results)

    # ── Step 6: Save comparison to cache ─────────────────────────────────────
    comparison_path = os.path.join(PROJECT_DIR, "data", "cache", "iter22_comparison.json")
    ...
    if decision == "SHIP":
        ...
        # Update holdout baseline with new candidate 2025-26 results
        new_baseline = {
            s: {
                "roi_pct": d["roi_pct"],
        ...
        update_holdout_baseline(new_baseline)
        print("\n  SHIP complete.")
    else:
