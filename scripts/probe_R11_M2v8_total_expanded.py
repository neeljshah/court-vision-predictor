...
OUT_JSON = os.path.join(DATA_CACHE, "probe_R11_M2v8_total_expanded_results.json")
...
def main():
    ...
    y = merged["total_pts_box"].astype(float).values
    ...
    naive_pred = merged["total_pts_box"].shift(1).rolling(5, min_periods=1).mean().fillna(
        merged["total_pts_box"].mean()
    ).values
