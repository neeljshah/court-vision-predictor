"""probe_R12_batch19_perm_importance_select.py — permutation-importance trimming.

Per pregame target, compute permutation importance on a single train/test split
of the canonical feature set, then re-run full 4-fold WF on three trimmed
feature sets:
  - drop_bottom_10pct  : keep top 90% by importance
  - drop_bottom_20pct  : keep top 80%
  - keep_top50         : keep top 50 features

Compares trimmed deltas to canonical baseline. Records beat_canonical.

Last sensible probe shot before pivoting to production wiring (stop 3/5).
"""
...
def _oof_stack_pred(merged, fc, label, kind, tr, ti):
    """One outer fold's B6-style OOF-stack final prediction."""
...
def _compute_perm_importance(merged, fc, label, kind, n_repeats=5):
    """Train a single LGB+XGB on the FIRST 3/4 of data, compute permutation
    importance on the last 1/4. Returns dict {feature → importance}."""
    from sklearn.inspection import permutation_importance
    from sklearn.metrics import make_scorer, brier_score_loss
    n = len(merged)
    split = (n * 3) // 4
    tr = list(range(0, split))
    te = list(range(split, n))
    X_tr = merged[fc].iloc[tr].values
    X_te = merged[fc].iloc[te].values
    y_all = merged[label].astype(int if kind == "bin" else float).values
    if kind == "reg":
        m_l = _lgb_reg(); m_l.fit(X_tr, y_all[tr])
        m_x = _xgb_reg(); m_x.fit(X_tr, y_all[tr])
        # Use LGB for permutation importance (faster + monotonic equivalence)
        scoring = "neg_mean_absolute_error"
        r = permutation_importance(m_l, X_te, y_all[te], n_repeats=n_repeats,
                                    random_state=42, n_jobs=1, scoring=scoring)
    else:
        m_l = _lgb_clf(); m_l.fit(X_tr, y_all[tr])
        # Use Brier (lower better) → negate via neg_brier_score
        scoring = "neg_brier_score"
        r = permutation_importance(m_l, X_te, y_all[te], n_repeats=n_repeats,
                                    random_state=42, n_jobs=1, scoring=scoring)
    importances = {fc[i]: float(r.importances_mean[i]) for i in range(len(fc))}
    return importances


def _trim_features(fc, importances, mode):
    """Return trimmed feature list. Mode: 'drop_bottom_10pct', 'drop_bottom_20pct', 'keep_top50'."""
    ranked = sorted(fc, key=lambda c: importances.get(c, 0.0), reverse=True)
    if mode == "drop_bottom_10pct":
        k = max(1, int(len(ranked) * 0.9))
    elif mode == "drop_bottom_20pct":
        k = max(1, int(len(ranked) * 0.8))
...
def run_wf_with_fc(merged, label, naive_pred, fc, kind):
    y_all = merged[label].astype(int if kind == "bin" else float).values
    n = len(merged)
    folds = []
...
def _summarize_reg(folds, name, label, n_features, meta, n_dropped):
    if not folds:
...
def _summarize_bin(folds, name, label, desc, n_features, n_games, pos_rate, meta, n_dropped):
    ...
    return {"probe": name, "kind": "binary", "label": label, "label_desc": desc,
            "n_features": n_features, "n_features_dropped": n_dropped,
...
def main():
    t0 = time.time()
    print("=" * 70, flush=True)
    print("R12 BATCH-19 — permutation-importance feature selection", flush=True)
    print("=" * 70, flush=True)
    ...
    for kind, label, desc in targets:
        ...
        fc_canon = ALL_FC[fc_name]
        naive = naive_l5_mean(label) if kind == "reg" else naive_l5_prop(label)
        print(f"\n[{label}] computing permutation importance on {len(fc_canon)} feats...", flush=True)
        t_imp = time.time()
        importances = _compute_perm_importance(merged, fc_canon, label, kind, n_repeats=5)
        print(f"  perm-importance done in {time.time()-t_imp:.1f}s", flush=True)
        # Log top 5 and bottom 5 features
        ranked = sorted(importances.items(), key=lambda x: x[1], reverse=True)
        top5 = [f"{n}:{v:.4f}" for n, v in ranked[:5]]
        bot5 = [f"{n}:{v:.4f}" for n, v in ranked[-5:]]
        print(f"  top5: {top5}", flush=True)
        print(f"  bot5: {bot5}", flush=True)

        for vmode in ["drop_bottom_10pct", "drop_bottom_20pct", "keep_top50"]:
            t_v = time.time()
            fc_trim = _trim_features(fc_canon, importances, vmode)
            n_dropped = len(fc_canon) - len(fc_trim)
            name = f"R12_B19_{vmode}_{label}"
            folds = run_wf_with_fc(merged, label, naive, fc_trim, kind)
            meta = {"variant": vmode, "fc_canonical": fc_name,
                    "n_features_kept": len(fc_trim), "n_features_dropped": n_dropped}
            if kind == "reg":
                out = _summarize_reg(folds, name, label, len(fc_trim) + 1, meta, n_dropped)
            else:
                y_all = merged[label].astype(int).values
                out = _summarize_bin(folds, name, label, desc, len(fc_trim) + 1,
                                      len(merged), float(np.mean(y_all)), meta, n_dropped)
            ...
            if out["kind"] == "regression":
                ...
                print(f"  {name}: {out['status']} kept={len(fc_trim)} dropped={n_dropped} "
