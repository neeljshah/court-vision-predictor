"""fetch_rs_closing_lines_fg3m_stl_blk.py — Iter 12a.

Backfill 2024-25 regular-season closing lines for FG3M + STL + BLK using
the same 4 game-nights that already have PTS/REB/AST data.

Strategy:
  - Same 10 events as REB+AST run (all have 5-7 bookmakers confirmed).
  - Markets: player_threes, player_steals, player_blocks (10 units each).
  - Hard cap: used_units must stay <= 656 (456 start + 200 task cap).
  - 6 events × 3 markets = 18 fetches × 10 = 180 units max.
...
# Hard budget cap: task start=456, allow 200 more → stop at 656
BUDGET_CAP = 656

# Markets for this run
MARKETS_TO_FETCH = ["player_threes", "player_steals", "player_blocks"]

# Stat meta: API market -> canonical stat code + gamelog column
...
# 6 events selected: 2 per date for Dec-20 + Feb-28, 1 each for Jan-25 + Apr-05
# Same snapshot_ts as used for REB/AST (pre-tip window confirmed)
# 6 events × 3 markets = 18 fetches × 10 = 180 units max
SELECTED_EVENTS: List[Tuple[str, str, str]] = [
    ...
    ("2025-01-25", "178af5284f2a47d4e8b7771bebab5da9", "2025-01-25T12:00:00Z"),
...
def main() -> None:
    budget = get_budget()
    print("\n" + "=" * 70)
    print("  Iter 12a — Regular-Season FG3M + STL + BLK Closing Lines Backfill")
    ...
    per_date_counters: Dict[str, Dict[str, int]] = defaultdict(
        lambda: {"events_fetched": 0, "fg3m_rows": 0, "stl_rows": 0, "blk_rows": 0,
                 "skipped": 0}
    )
    ...
    for date_str, event_id, snapshot_iso in SELECTED_EVENTS:
        ...
        for market_key in MARKETS_TO_FETCH:
            ...
            used_after = budget_after.get("used_units", "?")
            print(f"budget={used_after}/{budget_after.get('max_units')}", end="")
            ...
            for player_name, pdata in per_player.items():
                ...
                if dk not in existing_keys:
                    ...
                    rows_this_market += 1
                    per_date_counters[date_str][f"{stat_code}_rows"] += 1
        ...
        # Final budget check after each event
        budget_now = get_budget()
    ...
    total_fg3m = sum(v["fg3m_rows"] for v in per_date_counters.values())
    total_stl  = sum(v["stl_rows"]  for v in per_date_counters.values())
    total_blk  = sum(v["blk_rows"]  for v in per_date_counters.values())

    print(f"\n  {'Date':<15} {'Events':>8} {'FG3M':>6} {'STL':>6} {'BLK':>6}")
    print(f"  {'-'*15} {'-'*8} {'-'*6} {'-'*6} {'-'*6}")
    for date_str in sorted(per_date_counters):
        c = per_date_counters[date_str]
        print(f"  {date_str:<15} {c['events_fetched']:>8} "
              f"{c['fg3m_rows']:>6} {c['stl_rows']:>6} {c['blk_rows']:>6}")
    print(f"  {'TOTAL':<15} "
          f"{sum(c['events_fetched'] for c in per_date_counters.values()):>8} "
          f"{total_fg3m:>6} {total_stl:>6} {total_blk:>6}")

    print(f"\n  Total new CSV rows: FG3M={total_fg3m}, STL={total_stl}, BLK={total_blk}")
    print(f"  Total CSV rows (all stats): {len(all_rows)}")
    print(f"  API fetches made: {total_fetches}")
    print(f"  Final budget: {final_budget.get('used_units')}/{final_budget.get('max_units')}")

    # Per-stat breakdown in final CSV
    from collections import Counter
    stat_counts: Dict[str, int] = Counter(str(row.get("stat", "?")) for row in all_rows)
    print(f"\n  Rows by stat: {dict(sorted(stat_counts.items()))}")

    return total_fg3m, total_stl, total_blk, final_budget
