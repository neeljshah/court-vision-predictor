...
OUT_JSON = os.path.join(DATA_CACHE, "probe_R11_M2v4_away_pts_results.json")
...
def main():
    t0 = time.time()
    print("=" * 60, flush=True)
    print("probe_R11_M2v4_away_pts - game-level HOME team total points", flush=True)
    print("=" * 60, flush=True)
    ...
    y = merged["away_score"].astype(float).values

    # Naive baseline: rolling L5 mean of away_score (strictly prior games)
    print("\n[4] Naive baseline: L5 rolling mean of away_score ...", flush=True)
    naive_pred = merged["away_score"].shift(1).rolling(5, min_periods=1).mean().fillna(
        merged["away_score"].mean()
    ).values
