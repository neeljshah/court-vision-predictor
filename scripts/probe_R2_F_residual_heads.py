"""scripts/probe_R2_F_residual_heads.py -- R2-F residual heads probe (loop 5).

For each (pid, stat) at endQ3, if data/models/residual_heads/{stat}.lgb exists,
...
Usage:
    from scripts.probe_R2_F_residual_heads import treatment
    from scripts.improve_loop.scaffold import run_endq3_probe
    run_endq3_probe("R2_F_residual_heads", treatment)

CLI:
    python scripts/probe_R2_F_residual_heads.py
    python scripts/probe_R2_F_residual_heads.py --max-games 100
"""
...
def _load_heads() -> Dict[str, object]:
    """Load all available .lgb heads into memory (once)."""
...
def treatment(snap: dict) -> Dict[Tuple[int, str], float]:
    """Apply residual head corrections on top of BASELINE projections.
...
def main() -> int:
    ap = argparse.ArgumentParser(description="R2-F residual heads probe.")
    ...
    if n_heads == 0:
        print("  No residual heads found. Run train_residual_heads.py first.")
        return 1
