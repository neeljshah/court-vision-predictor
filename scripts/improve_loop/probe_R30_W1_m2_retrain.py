"""probe_R30_W1_m2_retrain.py — Third attempt at m2_family multi5 retrain.

R24_Q3 was blocked on missing pregame features.
R27_T1 only had 29 usable 2025-26 holdouts.
R30_W1 unblocked by:
  * R25_R1 backfill          -> 1225/1230 2025-26 with home_off_rtg
  * R26_S2 + R28_U1 + R29_V2 -> 100% linescore coverage for 2024-25 + 2025-26
  * R28_U2                   -> pace calibrated to NBA Stats truth
  * R29_V3                   -> synergy / sim_* fixes

Recipe is identical to R27_T1: 60-feature pregame set + LGB(seeds 42/7/100)
+ XGB(seeds 42/7) multi5 ensemble per target.

Ship gate (relaxed slightly given clean data, still rigorous):
...
Usage:
    python scripts/improve_loop/probe_R30_W1_m2_retrain.py
"""
...
ROOT_BACKUP_DIR = os.path.join(ROOT_DIR, "data", "models", "m2_family_R20_M7_backup_R30_W1")
ROOT_CACHE_PATH = os.path.join(ROOT_DIR, "data", "cache", "probe_R30_W1_results.json")
...
# ---------------------------------------------------------------------------
def train_ensemble(X: np.ndarray, y: np.ndarray) -> List:
...
def walk_forward_per_target(df: pd.DataFrame, feat_cols: List[str],
    ...
    Fold 1: train 2022-23           val 2023-24              [NEW-only, OLD saw it]
    Fold 2: train 2022-23..2023-24  val 2024-25              [NEW-only, OLD saw it]
    Fold 3: train 2022-23..2024-25  val 2025-26 (first half) [head-to-head]
    Fold 4: train + first half 2526 val 2025-26 (second half)[head-to-head]
    """
    df = df.copy()
...
def backup_old_artifacts() -> bool:
    ...
    try:
        ...
        print(f"[R30_W1] backup failed: {exc}", flush=True)
        return False
...
def persist_new_artifacts(df: pd.DataFrame, feat_cols: List[str]) -> Dict[str, list]:
    ...
    new_manifest = {
        ...
        "probe_ancestry":    {
            "round": "R30_W1",
            "predecessor": "R20_M7 (M2_family_v1, n_games=2836)",
            "trigger": "R25_R1 + R26_S2 + R28_U1 + R29_V2 + R28_U2 + R29_V3 all cleared the blocker; full 4-season clean corpus available.",
        },
...
def main() -> int:
    ...
    t0 = time.time()
    print(f"[R30_W1] root={ROOT_DIR}", flush=True)
    print(f"[R30_W1] models_dir={ROOT_MODELS_DIR}", flush=True)
    ...
    payload = {
        "probe":                 "R30_W1_m2_family_retrain",
    ...
    print(f"\n[R30_W1] wrote -> {ROOT_CACHE_PATH}", flush=True)
    print(f"[R30_W1] runtime: {runtime_min:.2f} min", flush=True)
